...
def get_clicked(pose):
  ...
  points = points_on_line(get_current_xyzw(), [x, y])
  print point
...
def check_cost(points, map=None, map_data=None):
  ...
  for point in points:
    # print point
    grid_x = np.uint32((point[0] - map.info.origin.position.x) / map.info.resolution)
    grid_y = np.uint32((point[1] - map.info.origin.position.y) / map.info.resolution)
...
def find_next_point(last, ref, min_area, r_scan):
  ...
  for ang in range(0,180):
    ...
    next_point = get_nearest_object_point(ref_point, ref, 100)
...
def get_nearest_object_point(source, ref, cost=95):
  points = points_on_line(source, ref)
  map = get_grid_map()
  map_data = occupancygrid_to_numpy(map)
  points_with_cost = check_cost(points, map, map_data)
  object_point = None
  ...
  while not rospy.is_shutdown():
    try:
        if get_current_xyzw()[0] != 0:
          ...
          # first phase for get first point
            object_point = get_nearest_object_point(get_current_xyzw(), ref_obj, 95)
...
def start_subs():
  ...
  subs = [cloud_sub]
  ts = message_filters.ApproximateTimeSynchronizer(subs, 1, 1)
  ts.registerCallback(scan_cb)
...
def main():
  try:
    ...
    cloud_publisher = rospy.Publisher('/good/depth/points', PointCloud2, queue_size=2)
